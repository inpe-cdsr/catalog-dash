...
@app.callback(
    ...
    [Input('download--date-picker-range', 'start_date'),
    Input('download--date-picker-range', 'end_date')])
def download__update_output_tables_and_charts_by_date_picker_range_values(start_date, end_date):

    logging.info('download__update_tables_and_charts()\n')

    logging.info('download__update_tables_and_charts() - start_date: %s', start_date)
    logging.info('download__update_tables_and_charts() - end_date: %s', end_date)

    # get just the date from [start|end]_dates
    xaxis_range = [start_date.split('T')[0], end_date.split('T')[0]]

    logging.info('download__update_tables_and_charts() - xaxis_range: %s\n', xaxis_range)
